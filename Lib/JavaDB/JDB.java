	...
	public static class Example {
		...
		public static StringBuilder Start() {
			...
			try {
				...
				try {
					...
					try {
						...
						try {
							...
							// Name()->Name, name()->name
							MemberVIP mvip = helper.getFirst(box.select(
						...
						box = db.cube();
						try {
							Member m = helper.getFirst(box.select(Member.class,
				...
				sb.append(BoxFileStreamConfig.RootPath + "||" + ex.getMessage()
						+ "\r\n");
			}
			return sb;
		}
			...
			try {
				...
				try {
					...
					try {
						...
						sb.append("\r\n*Begin Insert "
								+ helper.format(threadCount * objectCount)
								+ " objects");
						...
						sb.append("\r\nElapsed " + helper.getDou(sec)
								+ "s, AVG Insert " + helper.format(avg)
								+ " o/sec");
						System.gc();
						...
						sb.append("\r\nElapsed " + helper.getDou(sec)
								+ "s, AVG Lookup " + helper.format(avg)
								+ " o/sec");
						System.gc();
						System.runFinalization();
		...
		public static StringBuilder ReplicationSpeed(int time) {
			...
			try {
				...
				ReplicableServer server = new ReplicableServer();
				try {
					final Database masterA = server
							.getInstance(ServerID.MasterA_Address);
					final Database masterB = server
							.getInstance(ServerID.MasterB_Address);
					...
					BoxData.slaveReplicate(
							slaveA,
							((InMemoryBoxRecycler) masterA.getBoxRecycler())
									.GetBoxDataAndClear()).Assert();
					...
					for (int t = 0; t < time; t++) {
						...
						pool.awaitTermination(Integer.MAX_VALUE,
								TimeUnit.SECONDS);
						BoxData[] data = ((InMemoryBoxRecycler) masterA
